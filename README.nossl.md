...
Example for nginx, where <simvia-address> and <hub-address> are the addresses of your hosts where the services are running.

```
server {
    server_name <simvia.example.com>;
    listen 443 ssl http2;
    < YOUR SSL CONFIGURATION >;

    location /bff/api/v1/ws {
        proxy_pass http://<simvia-address>:3000/bff/api/v1/ws$is_args$args;
    ...
    location /bff/ {
        proxy_pass http://<simvia-address>:3000/bff/;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
    ...
    location / {
        proxy_pass http://<simvia-address>:3000/;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
